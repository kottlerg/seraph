...
│       ├── mod.rs              # Dispatch table, entry coordination; yield, exit, sleep, IPC-buffer and notification-binding syscalls
...
│       └── thread.rs           # Thread lifecycle syscalls (configure, start, stop, priority, affinity, regs, fault handler, sched split)
└── docs/
