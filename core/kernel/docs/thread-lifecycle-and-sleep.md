...
3. The cancel path: `sys_thread_stop` on a sleeper runs `cancel_ipc_block`, whose `IpcThreadState::None` arm does no source-lock work; the function then removes the TCB from the sleep list (before clearing `sleep_deadline` — the #117 order) and, on the remove win, stamps the park episode `INTERRUPTED` so the restarted sleeper returns `Interrupted` instead of reporting the truncated sleep as success. A stop that instead wins against the park commit (`ParkCommit::RefusedStop`) makes `sys_thread_sleep` deschedule in place and return `Interrupted` directly. Residual window: a cancel landing between the commit and `sleep_list_add` finds no entry and stamps nothing — the restart then reports the truncated sleep as `Ok(0)` (see the stop-vs-sleep-arming issue).
