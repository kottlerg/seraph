...
) -> !
{
    ...
    if is_userspace
    {
        ...
        if !tcb.is_null()
        {
            ...
            // hold; a refused commit yields the reason a teardown recorded.
            // SAFETY: tcb validated non-null.
            let reason = unsafe { crate::sched::exit_under_all_locks(tcb, 0x1000 + vector) }
                .exit_reason_or(0x1000 + vector);
