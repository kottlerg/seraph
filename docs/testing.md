...
aside, alters only comment lines in source files cannot change any binary
these runs boot, so none of the local host runs in this section are required
for it, whatever paths it touches. Source text a build embeds verbatim
