...
/// reply either way: the caller has already been woken — with the
/// synthetic `IPC_REPLY_TRANSFER_FAILED` label when the refusal happened
/// after it was claimed (a reply cap slot stale, repeated, revoke-pinned,
/// or unacceptable to its `CSpace`) — so the reply must not be retried.
///
/// # Safety
/// `ipc_buf` must point to the caller thread's 4 KiB-aligned IPC buffer
/// page as registered via `syscall::ipc_buffer_set`.
