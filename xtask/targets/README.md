...
| File | Floor | Used by |
|---|---|---|
| `x86_64-seraph-none.json` | x86-64 baseline, soft-float | Kernel (`os: none`, static-PIE) |
...
| `riscv64imac-seraph-none.json` | RV64IMAC, soft-float, lp64 | Kernel (`os: none`, static-PIE) |
...
loaders can randomize each image's base (ASLR, #39). The kernel targets are
also `relocation-model: pic` static-PIE (`ET_DYN`, `RELATIVE`-only; x86-64
uses the `small` code model): the bootloader chooses the kernel's load bias
and applies its relocations before handoff (KASLR, #252).
